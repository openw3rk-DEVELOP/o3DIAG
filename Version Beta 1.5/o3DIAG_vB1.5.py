...
\___/ .__/\__/_//_/__,__/____/_/ /_/\_\  /___/_/|_/ |___/___/_/|_/ /_/    
   /_/ o3DIAG Beta 1.5 - https://o3diag.openw3rk.de | https://openw3rk.de      
--------------------------------------------------------------------------
...
"""
    ...
    def __init__(self, root):
        self.root = root
        root.title("o3DIAG (version Beta 1.5) (for OBD / ELM327)")
        ...
        self.dtc_map = {}
        self.o3script_filename = "o3DIAG_pcodes_list_german.o3script"
        self.load_dtc_map()
    ...
    def connect(self):
        ...
        try:
            ...
            messagebox.showwarning("Baud", "Ungültige Baudrate")
            return
        if not port:
            messagebox.showwarning("Port", "Kein Port ausgewählt")
            return
    ...
    def send_command(self, cmd: str):
        if not self.connected:
            self.log("Nicht verbunden – Befehl nicht gesendet.")
            return
    ...
    def init_adapter(self):
        if not self.connected:
            self.log("Nicht verbunden: bitte zuerst Connect.")
            return
    ...
    def clear_dtcs(self):
        if messagebox.askyesno("DTC löschen", "Gespeicherte Fehlercodes wirklich löschen? (OBD Mode 04)"):
    ...
    def process_response(self, data: str):
        ...
        if "43" in clean:
            ...
            if not dtcs:
                if re.search(r'\b43\s*00\b', clean, flags=re.I):
                    self.log("Keine Fehlercodes gespeichert.")
                return
            self.log("Fehlercodes:")
            for code in dtcs:
                desc = self.lookup_dtc(code)
                if desc:
                    self.log(f"  {code} – {desc}")
                else:
                    self.log(f"  {code} – (keine Beschreibung gefunden)")

        if "NO DATA" in clean.upper():
            self.log("Hinweis: NO DATA – PID/Mode nicht unterstützt oder keine aktuellen Werte.")
    def load_dtc_map(self):
        ...
        try:
            ...
            self.log(f"P-Code Liste nicht gefunden: {self.o3script_filename}")
            self.dtc_map = {}
            return
        except Exception as e:
            self.log(f"P-Code Liste konnte nicht gelesen werden: {e}")
            self.dtc_map = {}
            return

        self.dtc_map = new_map
        self.log(f"P-Code Liste geladen: {len(self.dtc_map)} Einträge")
